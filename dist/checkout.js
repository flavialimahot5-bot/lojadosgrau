...
const {repriceSaleCart}=await import('/sale-pricing.js');cart=await repriceSaleCart(cart,byId);$('#loading').hidden=true;if(!cart.length){$('#empty').hidden=false;return}$('#checkout-layout').hidden=false;
const subtotal=cart.reduce((sum,i)=>sum+i.quantity*i.price,0);document.querySelectorAll('[data-subtotal]').forEach(e=>e.textContent=cash(subtotal));$('#item-count').textContent=cart.reduce((sum,i)=>sum+i.quantity,0);window.checkoutCart=cart;window.dispatchEvent(new Event('checkout:ready'));
