...
import {CheckoutError,configuration,priceCart,customerData,iron,sign,equal} from './payments.js';
...
function decode(id,token){
 ...
 if(data.id!==id||!['quote','receipt'].includes(data.kind))fail(403,'Acesso ao pedido inválido.');
...
function verify(receipt,data){
 ...
 const state={paid:'paid',pending:'pending',waiting_payment:'pending',canceled:'canceled',cancelled:'canceled',refunded:'refunded',expired:'expired',refused:'canceled'}[data.payment_status||data.status]||'processing';
...
async function present(receipt){
 return{id:receipt.id,accessToken:encode(receipt),state:receipt.state,amount:receipt.amount,subtotal:receipt.subtotal,shipping:receipt.shipping,shippingMethod:receipt.shippingMethod,pixCode:receipt.pixCode,qrCode:receipt.pixCode?await QRCode.toDataURL(receipt.pixCode,{width:320,margin:2,errorCorrectionLevel:'M'}):null,expiresAt:receipt.expiresAt};
}
const unknown=order=>({id:order.id,state:'unknown',amount:order.amount,subtotal:order.subtotal,shipping:order.shipping,shippingMethod:order.shippingMethod});
...
export async function create(id,token,raw){
 const q=decode(id,token);if(q.kind==='receipt')return view(id,token);
 ...
 const result=(async()=>{
  ...
  const payload={amount:q.amount,offer_hash:process.env.IRONPAY_OFFER_HASH.trim(),payment_method:'pix',installments:1,customer,cart:priced.items.map(i=>({product_hash:product,title:(i.title+' '+Object.values(i.options).join(' / ')).slice(0,250),price:i.price,quantity:i.quantity,operation_type:1,tangible:true})),expire_in_days:1,transaction_origin:'api'};
  if(q.shipping)payload.cart.push({product_hash:product,title:'Frete',price:q.shipping,quantity:1,operation_type:1,tangible:false});
  try{
   const data=await iron('/transactions',payload);
   if(typeof data.hash!=='string'||!/^[a-zA-Z0-9_-]{1,120}$/.test(data.hash))return unknown(q);
