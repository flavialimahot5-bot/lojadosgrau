...
export function allowedCheckoutOrigin(origin,env=process.env){
 if(typeof origin!=='string')return false;
 const allowed=new Set(['https://lojadosgrau.vercel.app','https://gsuplementsbr.vercel.app']);
 try{const url=new URL(configuration(env).site);if(url.protocol==='https:'&&!url.username&&!url.password)allowed.add(url.origin)}catch{}
